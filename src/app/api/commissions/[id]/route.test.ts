...
function makeGetRequest(): Request {
  return new Request('http://localhost/api/commissions/comm-001', { method: 'GET' })
}

function makePatchRequest(body: unknown): Request {
  return new Request('http://localhost/api/commissions/comm-001', {
    method: 'PATCH',
...
describe('PATCH /api/commissions/[id]', () => {
  it('returns 401 if not authenticated', async () => {
    mockUser = null
    const res = await PATCH(makePatchRequest({ status: 'approved' }) as never, { params })
    expect(res.status).toBe(401)
  })
  ...
  it('updates status to paid', async () => {
    ...
    const res = await PATCH(makePatchRequest({ status: 'paid' }) as never, { params })
    expect(res.status).toBe(200)
  })

  it('updates status to cancelled', async () => {
    mockUpdatedCommission = { ...mockCommission, status: 'cancelled' }
    const res = await PATCH(makePatchRequest({ status: 'cancelled' }) as never, { params })
    expect(res.status).toBe(200)
  })

  it('returns 400 for invalid status value', async () => {
    const res = await PATCH(makePatchRequest({ status: 'invalid' }) as never, { params })
    expect(res.status).toBe(400)
  })

  it('accepts optional paid_at', async () => {
    mockUpdatedCommission = { ...mockCommission, status: 'paid', paid_at: '2026-01-30T12:00:00Z' }
    const res = await PATCH(makePatchRequest({ status: 'paid', paid_at: '2026-01-30T12:00:00Z' }) as never, { params })
    expect(res.status).toBe(200)
  })
