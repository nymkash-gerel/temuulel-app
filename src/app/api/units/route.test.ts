...
describe('GET /api/units', () => {
  it('returns 401 if user is not authenticated', async () => {
    mockUser = null
    const res = await GET(makeRequest('http://localhost/api/units'))
  ...
  it('supports status filter', async () => {
    ...
    mockDataCount = 1
    const res = await GET(makeRequest('http://localhost/api/units?status=available'))
    const json = await res.json()
    expect(res.status).toBe(200)
  ...
  it('supports unit_type filter', async () => {
    ...
    mockDataCount = 1
    const res = await GET(makeRequest('http://localhost/api/units?unit_type=suite'))
    const json = await res.json()
    expect(res.status).toBe(200)
    expect(json.data).toHaveLength(1)
  })
  ...
  it('supports combined status and unit_type filters', async () => {
    ...
    mockDataCount = 1
    const res = await GET(makeRequest('http://localhost/api/units?status=occupied&unit_type=penthouse'))
    const json = await res.json()
    expect(res.status).toBe(200)
    expect(json.data).toHaveLength(1)
  })
  ...
  it('ignores invalid status filter values', async () => {
    mockData = [{ id: 'unit-1' }]
    mockDataCount = 1
    const res = await GET(makeRequest('http://localhost/api/units?status=invalid_status'))
    const json = await res.json()
    expect(res.status).toBe(200)
    expect(json.data).toHaveLength(1)
  })
  ...
  it('ignores invalid unit_type filter values', async () => {
    mockData = [{ id: 'unit-1' }]
    mockDataCount = 1
    const res = await GET(makeRequest('http://localhost/api/units?unit_type=invalid_type'))
    const json = await res.json()
    expect(res.status).toBe(200)
    expect(json.data).toHaveLength(1)
  })
  ...
  it('supports pagination parameters', async () => {
    mockData = [{ id: 'unit-10' }]
    mockDataCount = 50
    const res = await GET(makeRequest('http://localhost/api/units?limit=10&offset=20'))
    const json = await res.json()
    expect(res.status).toBe(200)
...
describe('POST /api/units', () => {
  ...
  it('returns 401 if not authenticated', async () => {
    mockUser = null
    const res = await POST(makeRequest('http://localhost/api/units', validBody))
    expect(res.status).toBe(401)
  })
  ...
  it('returns 400 for invalid JSON body', async () => {
    const req = createTestRequest('http://localhost/api/units', {
      method: 'POST',
