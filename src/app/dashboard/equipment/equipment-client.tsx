...
export default function EquipmentClient({ storeId }: { storeId: string }) {
    ...
    <div className="p-6">
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <div className="flex-1">
            <input
              ...
              className="w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-xl text-white placeholder-gray-400 focus:outline-none focus:border-blue-500/30 focus:ring-2 focus:ring-blue-500/20 transition-all"
            />
          </div>
          <div className="flex items-center gap-3 flex-wrap">
            <select
              ...
              className="px-4 py-3 bg-gray-700 border border-gray-600 rounded-xl text-white focus:outline-none focus:border-blue-500/30 focus:ring-2 focus:ring-blue-500/20 transition-all"
            >
          ...
          <div className="bg-gray-800 border border-gray-700 rounded-2xl p-6 max-w-lg w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Жишээ: Гүйдэг зам Technogym" className="w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-xl text-white placeholder-gray-400 focus:outline-none focus:border-blue-500/30 focus:ring-2 focus:ring-blue-500/20 transition-all" required />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm text-gray-400 mb-1">Төрөл *</label>
                  <select value={form.equipment_type} onChange={(e) => setForm({ ...form, equipment_type: e.target.value })} className="w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-xl text-white focus:outline-none focus:border-blue-500/30 focus:ring-2 focus:ring-blue-500/20 transition-all">
                    <option value="cardio">Кардио</option>
                    <option value="strength">Хүч чадал</option>
                    <option value="free_weights">Чөлөөт жин</option>
                    <option value="flexibility">Уян хатан</option>
                ...
                <div>
                  ...
                  <select value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value })} className="w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-xl text-white focus:outline-none focus:border-blue-500/30 focus:ring-2 focus:ring-blue-500/20 transition-all">
                    <option value="available">Ашиглах боломжтой</option>
                    <option value="in_use">Ашиглагдаж буй</option>
                    <option value="maintenance">Засвар дээр</option>
                    <option value="out_of_order">Эвдэрсэн</option>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={form.serial_number} onChange={(e) => setForm({ ...form, serial_number: e.target.value })} placeholder="SN-12345" className="w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-xl text-white placeholder-gray-400 focus:outline-none focus:border-blue-500/30 focus:ring-2 focus:ring-blue-500/20 transition-all" />
                </div>
                <div>
                  <label className="block text-sm text-gray-400 mb-1">Байршил</label>
                  <input type="text" value={form.location} onChange={(e) => setForm({ ...form, location: e.target.value })} placeholder="1-р давхар, Кардио зал" className="w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-xl text-white placeholder-gray-400 focus:outline-none focus:border-blue-500/30 focus:ring-2 focus:ring-blue-500/20 transition-all" />
                </div>
              </div>
              <div>
                <label className="block text-sm text-gray-400 mb-1">Худалдан авсан огноо</label>
                <input type="date" value={form.purchase_date} onChange={(e) => setForm({ ...form, purchase_date: e.target.value })} className="w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-xl text-white focus:outline-none focus:border-blue-500/30 focus:ring-2 focus:ring-blue-500/20 transition-all" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm text-gray-400 mb-1">Сүүлийн засвар</label>
                  <input type="date" value={form.last_maintenance_date} onChange={(e) => setForm({ ...form, last_maintenance_date: e.target.value })} className="w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-xl text-white focus:outline-none focus:border-blue-500/30 focus:ring-2 focus:ring-blue-500/20 transition-all" />
                </div>
                <div>
                  <label className="block text-sm text-gray-400 mb-1">Дараагийн засвар</label>
                  <input type="date" value={form.next_maintenance_date} onChange={(e) => setForm({ ...form, next_maintenance_date: e.target.value })} className="w-full px-4 py-3 bg-gray-700 border border-gray-600 rounded-xl text-white focus:outline-none focus:border-blue-500/30 focus:ring-2 focus:ring-blue-500/20 transition-all" />
                </div>
              </div>
              <div className="flex gap-3 pt-2">
                ...
                <button type="submit" disabled={saving} className="flex-1 px-4 py-3 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-xl disabled:opacity-50 transition-all">{saving ? 'Хадгалж байна...' : editingId ? 'Шинэчлэх' : 'Нэмэх'}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {filtered.length > 0 ? (
              ...
              {filtered.map((eq) => {
                ...
                return (
                  <tr key={eq.id} onClick={() => router.push(`/dashboard/equipment/${eq.id}`)} className="border-b border-gray-700/50 hover:bg-gray-700/50 transition-all cursor-pointer">
                    <td className="py-3 px-3 md:py-4 md:px-6"><span className="text-white font-medium">{eq.name}</span></td>
                    <td className="py-3 px-3 md:py-4 md:px-6"><span className="text-gray-300 text-sm">{EQUIPMENT_TYPE_LABELS[eq.equipment_type] || eq.equipment_type}</span></td>
                    <td className="py-3 px-3 md:py-4 md:px-6"><span className="text-gray-400 font-mono text-sm">{eq.serial_number || '-'}</span></td>
                    ...
                    <td className="py-3 px-3 md:py-4 md:px-6"><span className="text-gray-300 text-sm">{eq.location || '-'}</span></td>
                    <td className="py-3 px-3 md:py-4 md:px-6"><span className="text-gray-400 text-sm">{eq.purchase_date ? new Date(eq.purchase_date).toLocaleDateString('mn-MN') : '-'}</span></td>
                    <td className="py-3 px-3 md:py-4 md:px-6">
                      {eq.next_maintenance_date ? (
                        <span className={`text-sm ${new Date(eq.next_maintenance_date) < new Date() ? 'text-red-400 font-medium' : 'text-gray-400'}`}>
                    ...
                    <td className="py-3 px-3 md:py-4 md:px-6 text-right">
                      <button onClick={(e) => { e.stopPropagation(); startEdit(eq) }} className="px-3 py-1 text-xs bg-gray-700 text-gray-300 rounded-lg hover:bg-gray-600 transition-all">Засах</button>
                    </td>
                  </tr>
                )
              })}
        ...
        <div className="bg-gray-800 border border-gray-700 rounded-2xl p-12 text-center">
          ...
          <p className="text-gray-400 mb-6 max-w-md mx-auto">Тоног төхөөрөмжөө бүртгэж, засвар үйлчилгээг хянана уу</p>
          <button onClick={() => { resetForm(); setShowForm(true) }} className="inline-flex items-center gap-2 px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-xl transition-all">
