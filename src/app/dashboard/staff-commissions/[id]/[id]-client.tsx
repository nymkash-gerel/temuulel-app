...
export default function StaffCommissionDetailClient({ storeId }: { storeId: string }) {
  ...
  const [commission, setCommission] = useState<StaffCommissionDetail | null>(null)
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 404) {
          router.push('/dashboard/staff-commissions')
          return
        }
  ...
  if (error) {
    return (
      <div className="text-center py-12">
        ...
        <Link href="/dashboard/staff-commissions" className="text-blue-400 hover:text-blue-300 mt-2 inline-block">
          Буцах
        </Link>
      </div>
    )
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Link
            href="/dashboard/staff-commissions"
          ...
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-2xl font-bold text-white">Ажилтны шимтгэл</h1>
              {isEditing ? (
                <select
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-4">
            <h3 className="text-sm text-slate-400 font-medium mb-3">Тооцоолол</h3>
            <div className="space-y-2">
              <div className="flex justify-between">
                <span className="text-xs text-slate-500">Борлуулалт</span>
                <span className="text-white text-sm">{formatPrice(commission.sale_amount)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-xs text-slate-500">Хувь</span>
                <span className="text-white text-sm">{formatPercent(commission.commission_rate)}</span>
              </div>
              <div className="border-t border-white/[0.06] pt-2 flex justify-between">
                <span className="text-xs text-slate-400 font-medium">Шимтгэл</span>
                <span className="text-blue-400 text-sm font-medium">{formatPrice(commission.commission_amount)}</span>
              </div>
            </div>
          </div>
        </div>
