...
export default function InspectionDetailClient({ initialInspection, initialProjectName, inspectionId }: Props) {
  ...
  const [findings, setFindings] = useState(initialInspection.findings || initialInspection.required_corrections || '')
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-8">
        <div className="flex items-center gap-4">
          <Link href="/dashboard/inspections" className="p-2 text-slate-400 hover:text-white hover:bg-white/[0.08] rounded-lg transition-all">
            &larr;
          </Link>
          <div>
          ...
          {inspection.status !== 'cancelled' && inspection.status !== 'passed' && inspection.status !== 'failed' && (
            <>
              ...
              <button
                ...
                disabled={saving}
                className="px-4 py-2.5 bg-white/[0.04] hover:bg-white/[0.08] border border-white/[0.06] text-slate-300 rounded-xl text-sm transition-all disabled:opacity-50"
              >
                Цуцлах
              </button>
            </>
