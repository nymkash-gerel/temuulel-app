...
export default function ServicesClient({ storeId }: { storeId: string }) {
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-6">
        <div className="flex-1">
          <input
            ...
            placeholder="Хайх..."
            className="w-full px-4 py-3 bg-white/[0.03] border border-white/[0.06] rounded-xl text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-pink-500"
          />
        </div>
        <select
          value={statusFilter}
          onChange={(e) => setStatusFilter(e.target.value)}
          className="px-4 py-3 bg-white/[0.03] border border-white/[0.06] rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-pink-500"
        >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredServices.map((service) => (
            <Link
              ...
              className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-6 hover:border-pink-500/50 transition-all group"
            >
              {/* Image */}
              {service.images && service.images.length > 0 ? (
                <div className="w-full h-32 rounded-xl overflow-hidden mb-4 bg-white/[0.06]">
                  <img
