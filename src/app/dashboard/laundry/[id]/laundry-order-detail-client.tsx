...
interface LaundryOrderDetailClientProps {
  initialOrder: LaundryOrder
  orderId: string
}

export default function LaundryOrderDetailClient({ initialOrder, orderId }: LaundryOrderDetailClientProps) {
  const router = useRouter()

  const [order, setOrder] = useState<LaundryOrder>(initialOrder)
          ...
          <div>
            ...
            <div className="mt-2 flex items-center gap-2 flex-wrap">
              ...
              {!isEditing ? (
                <button
                  onClick={startEdit}
                  className="px-4 py-2 bg-white/[0.04] hover:bg-white/[0.08] border border-white/[0.06] text-white rounded-xl text-sm font-medium transition-all"
                >
                  Засах
                </button>
              ) : (
                <>
                  <button
                    ...
                    disabled={saving}
                    className="px-4 py-2 bg-white/[0.04] hover:bg-white/[0.08] border border-white/[0.06] text-slate-300 rounded-xl text-sm font-medium transition-all disabled:opacity-50"
                  >
                    Болих
                  </button>
                  <button
