...
export default function DeliveryMapClient({ initialStoreId }: DeliveryMapClientProps) {
  ...
  useEffect(() => {
    async function load() {
      await fetchDrivers(storeId)
      setLoading(false)
    }
    load()
  }, [storeId, fetchDrivers])
  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        ...
        <div className="flex gap-3">
          <button
            ...
            className="px-4 py-2.5 bg-white/[0.04] hover:bg-white/[0.08] border border-white/[0.06] text-white rounded-xl transition-all"
          >
            Шинэчлэх
          </button>
          <Link
      ...
      {/* Map */}
      <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl overflow-hidden" style={{ height: '500px' }}>
        <MapContainer
          center={UB_CENTER}
          zoom={12}
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {drivers.map(driver => (
          <div key={driver.id} className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-4">
