...
export default function DamageReportsClient({ initialStoreId }: DamageReportsClientProps) {
  ...
  const loadReports = useCallback(async (sid: string) => {
    ...
    if (typeFilter) query = query.eq('damage_type', typeFilter)
  ...
  const stats = useMemo(() => {
    ...
    const unresolved = reports.filter(r => r.status === 'reported' || r.status === 'assessed' || r.status === 'charged').length
      ...
      <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-4 mb-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="w-full px-4 py-3 bg-white/[0.04] border border-white/[0.06] rounded-xl text-white focus:outline-none focus:border-pink-500 transition-all">
          ...
          <div>
            ...
            <select value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)} className="w-full px-4 py-3 bg-white/[0.04] border border-white/[0.06] rounded-xl text-white focus:outline-none focus:border-pink-500 transition-all">
        ...
        {(statusFilter || typeFilter) && (
          <div className="mt-3 pt-3 border-t border-white/[0.06]">
            <button onClick={() => { setStatusFilter(''); setTypeFilter('') }} className="text-sm text-pink-400 hover:text-pink-300 transition-all">Clear all filters</button>
          </div>
        )}
      </div>
      ...
      {reports.length > 0 ? (
          ...
          <table className="w-full min-w-[1000px]">
            ...
            <tbody>
              {reports.map((report) => {
                ...
                const guestName = report.guests ? `${report.guests.first_name} ${report.guests.last_name}` : '-'
                return (
                  <tr key={report.id} className="border-b border-white/[0.04] hover:bg-white/[0.04] transition-all">
                    <td className="py-3 px-3 md:py-4 md:px-6"><span className="text-white font-medium">{report.units?.unit_number || '-'}</span></td>
                    <td className="py-3 px-3 md:py-4 md:px-6">
                      <div>
                        <p className="text-white">{guestName}</p>
                        {report.reservations && <p className="text-slate-400 text-xs">{formatDate(report.reservations.check_in)} - {formatDate(report.reservations.check_out)}</p>}
                      </div>
                    </td>
                    <td className="py-3 px-3 md:py-4 md:px-6"><span className="text-slate-300 capitalize">{report.damage_type.replace(/_/g, ' ')}</span></td>
                    <td className="py-3 px-3 md:py-4 md:px-6"><p className="text-slate-300 text-sm truncate max-w-[200px]" title={report.description}>{report.description}</p></td>
                    <td className="py-3 px-3 md:py-4 md:px-6 text-right"><span className="text-white">{report.estimated_cost != null ? formatPrice(report.estimated_cost) : '-'}</span></td>
                    <td className="py-3 px-3 md:py-4 md:px-6 text-right"><span className="text-white font-medium">{report.charged_amount > 0 ? formatPrice(report.charged_amount) : '-'}</span></td>
                    ...
                    <td className="py-3 px-3 md:py-4 md:px-6"><span className="text-slate-400 text-sm">{formatDate(report.created_at)}</span></td>
                  </tr>
                )
              })}
            </tbody>
        ...
        <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-12 text-center">
          ...
          <p className="text-slate-400 mb-6 max-w-md mx-auto">
            {statusFilter || typeFilter ? 'No reports match your current filters. Try adjusting the filters.' : 'Damage reports will appear here when incidents are logged for your property.'}
          </p>
          {(statusFilter || typeFilter) && (
            <button onClick={() => { setStatusFilter(''); setTypeFilter('') }} className="px-4 py-2 bg-white/[0.04] hover:bg-white/[0.08] border border-white/[0.06] text-white rounded-lg transition-all text-sm">Clear filters</button>
          )}
        </div>
      )}
    </div>
