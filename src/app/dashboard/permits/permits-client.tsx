...
export default function PermitsClient({ storeId }: { storeId: string }) {
    ...
    async function init() {
      ...
      if (storeId) {
        const [, projectsRes] = await Promise.all([
          loadPermits(),
          supabase.from('projects').select('id, name').eq('store_id', storeId).order('name'),
        ])
          ...
          <table className="w-full min-w-[1000px]">
            ...
            <tbody>
              {permits.map((permit) => {
                ...
                return (
                  <tr key={permit.id} className="border-b border-white/[0.04] hover:bg-white/[0.04] transition-all">
