...
interface DailyLogDetailClientProps {
  logId: string
  initialLog: DailyLogDetail | null
}

export default function DailyLogDetailClient({ logId, initialLog }: DailyLogDetailClientProps) {
  const router = useRouter()

  const [loading, setLoading] = useState(!initialLog)
  const [log, setLog] = useState<DailyLogDetail | null>(initialLog)
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="lg:col-span-2 bg-white/[0.03] border border-white/[0.06] rounded-xl p-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              {isEditing ? (
                <input type="number" value={editData.crew_count as string | number} onChange={e => setEditData({ ...editData, crew_count: e.target.value })} className={`${inputClassName} mt-1`} min="0" />
              ) : (
            ...
            <div>
              ...
              {isEditing ? (
                <input type="number" value={editData.hours_worked as string | number} onChange={e => setEditData({ ...editData, hours_worked: e.target.value })} className={`${inputClassName} mt-1`} min="0" step="0.5" />
              ) : (
            ...
            <div>
              ...
              {log.projects ? (
                <Link href={`/dashboard/projects/${log.projects.id}`} className="text-blue-400 hover:text-blue-300 text-sm mt-1 inline-block transition-all">
                  {log.projects.name}
                </Link>
              ) : (
          ...
          <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-4">
            ...
            {log.projects ? (
              <div>
                ...
                <Link href={`/dashboard/projects/${log.projects.id}`} className="text-blue-400 hover:text-blue-300 text-xs transition-all">
                  Дэлгэрэнгүй
                </Link>
              </div>
            ) : (
      ...
      <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-6">
        ...
        {isEditing ? (
          <textarea value={editData.work_summary as string} onChange={e => setEditData({ ...editData, work_summary: e.target.value })} className={`${inputClassName} min-h-[100px]`} rows={4} />
        ) : (
          <p className="text-sm text-slate-300 whitespace-pre-wrap leading-relaxed">{log.work_summary || 'Ажлын товчоо оруулаагүй'}</p>
        )}
      </div>
      ...
      <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-6">
        ...
        {isEditing ? (
          <textarea value={editData.issues as string} onChange={e => setEditData({ ...editData, issues: e.target.value })} className={`${inputClassName} min-h-[100px]`} rows={4} />
        ) : (
          <p className="text-sm text-slate-300 whitespace-pre-wrap leading-relaxed">{log.issues || 'Асуудал бүртгэгдээгүй'}</p>
        )}
      </div>

      {/* Safety Incidents */}
      <div className={`border rounded-xl p-6 ${hasSafetyIncidents ? 'bg-red-900/20 border-red-800' : 'bg-white/[0.03] border-white/[0.06]'}`}>
        <h3 className={`text-sm font-medium mb-3 ${hasSafetyIncidents ? 'text-red-400' : 'text-slate-400'}`}>Аюулгүй байдлын осол</h3>
        {isEditing ? (
          <textarea value={editData.safety_incidents as string} onChange={e => setEditData({ ...editData, safety_incidents: e.target.value })} className={`${inputClassName} min-h-[100px]`} rows={4} />
        ) : (
          <p className={`text-sm whitespace-pre-wrap leading-relaxed ${hasSafetyIncidents ? 'text-red-300' : 'text-slate-300'}`}>{log.safety_incidents || 'Осол бүртгэгдээгүй'}</p>
        )}
      </div>
      ...
      <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-6">
        ...
        <p className="text-sm text-slate-300 whitespace-pre-wrap leading-relaxed">{log.materials_used || 'Материал бүртгэгдээгүй'}</p>
      </div>

      {/* Visitor Log */}
      <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-6">
        <h3 className="text-sm text-slate-400 font-medium mb-3">Зочдын бүртгэл</h3>
        <p className="text-sm text-slate-300 whitespace-pre-wrap leading-relaxed">{log.visitor_log || 'Зочин бүртгэгдээгүй'}</p>
      </div>
      ...
      <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-6">
        ...
        {isEditing ? (
          ...
          <p className="text-sm text-slate-300 whitespace-pre-wrap leading-relaxed">{log.notes || 'Тэмдэглэл байхгүй'}</p>
        )}
      </div>
