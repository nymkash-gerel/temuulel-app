...
interface NewEncounterClientProps {
  initialStoreId: string
}

export default function NewEncounterClient({ initialStoreId }: NewEncounterClientProps) {
  ...
  useEffect(() => {
    const init = async () => {
      const [pRes, sRes] = await Promise.all([
        supabase.from('patients').select('id, first_name, last_name').eq('store_id', initialStoreId).order('first_name'),
        supabase.from('staff').select('id, name').eq('store_id', initialStoreId).order('name'),
      ])
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-8">
        <Link href="/dashboard/encounters" className="p-2 text-slate-400 hover:text-white hover:bg-white/[0.08] rounded-lg">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <select value={patientId} onChange={(e) => setPatientId(e.target.value)}
                      className="w-full px-4 py-3 bg-white/[0.04] border border-white/[0.06] rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-blue-500" required>
