...
export default function PharmacyClient({ storeId }: { storeId: string }) {
      ...
      {prescriptions.length > 0 ? (
        <div className="space-y-4">
          {prescriptions.map(rx => (
            <div key={rx.id} className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-6">
              ...
              {rx.notes && (
                <p className="text-slate-400 text-sm mt-3 border-t border-white/[0.06] pt-3">{rx.notes}</p>
              )}
            </div>
          ))}
        </div>
