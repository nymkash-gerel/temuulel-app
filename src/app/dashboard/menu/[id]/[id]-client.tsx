...
export default function MenuItemDetailClient({ storeId }: { storeId: string }) {
            ...
            {menuItem.images && menuItem.images.length > 0 ? (
              <div className="grid grid-cols-2 gap-2">
                {menuItem.images.map((img, i) => (
                  <div key={i} className="aspect-square rounded-lg overflow-hidden bg-white/[0.06]">
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all ${
                    (menuItem as unknown as Record<string, boolean>)[key]
                      ...
                      : 'bg-white/[0.04] border-white/[0.08] text-slate-400 hover:border-slate-500'
                  }`}
                >
                  {icon} {label}
                </button>
                  ...
                  <button
                    ...
                    className={`px-2 py-1 rounded text-xs transition-all ${
                      menuItem.spicy_level === level
                        ...
                        : 'bg-white/[0.04] border border-white/[0.06] text-slate-500 hover:border-slate-500'
                    }`}
                  >
                    {label}
                  </button>
