...
export default function LeasesClient({ initialLeases, storeId }: LeasesClientProps) {
  const supabase = useMemo(() => createClient(), [])

  const [loading, setLoading] = useState(false)
  const [leases, setLeases] = useState<LeaseRow[]>(initialLeases)
          ...
          <table className="w-full min-w-[900px]">
            ...
            <tbody>
              {leases.map((lease) => {
                ...
                return (
                  <tr key={lease.id} className="border-b border-white/[0.04] hover:bg-white/[0.04] transition-all">
