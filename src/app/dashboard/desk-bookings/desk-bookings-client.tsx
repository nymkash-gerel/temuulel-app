...
interface DeskBookingsClientProps {
  initialStoreId: string
}

export default function DeskBookingsClient({ initialStoreId }: DeskBookingsClientProps) {
  ...
  }, [supabase, statusFilter, initialStoreId])
  ...
  useEffect(() => {
    if (loading) return
    const reload = async () => { await loadBookings() }
    reload()
          ...
          <table className="w-full min-w-[950px]">
            ...
            <tbody>
              {bookings.map((b) => {
                const sc = STATUS_CONFIG[b.status] || { label: b.status, color: 'bg-slate-500/20 text-slate-400' }
                return (
                  <tr key={b.id} className="border-b border-white/[0.04] hover:bg-white/[0.04] transition-all">
