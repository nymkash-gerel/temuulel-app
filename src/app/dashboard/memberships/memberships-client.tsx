...
export default function MembershipsClient({ storeId }: { storeId: string }) {
          ...
          <table className="w-full min-w-[800px]">
            ...
            <tbody>
              {plans.map((plan) => {
                ...
                return (
                  <tr key={plan.id} onClick={() => router.push(`/dashboard/memberships/${plan.id}`)} className="border-b border-white/[0.04] hover:bg-white/[0.04] transition-all cursor-pointer">
                        ...
                        {plan.benefits && plan.benefits.length > 0 && (
                          <div className="flex flex-wrap gap-1 mt-1">
                            {plan.benefits.slice(0, 3).map((b, i) => (
                              <span key={i} className="px-2 py-0.5 bg-white/[0.04] text-slate-300 text-xs rounded-full">
                                {b}
                              </span>
                            ))}
