...
export default function SubscriptionsClient({ storeId }: { storeId: string }) {
    ...
    async function load() {
      ...
      if (storeId) {
        ...
        const [subsRes, customersRes] = await Promise.all([
          fetch(`/api/subscriptions?store_id=${storeId}`),
          supabase
            .from('customers')
            .select('id, name')
            .eq('store_id', storeId)
        ...
        <div className="flex flex-col md:flex-row gap-4">
          ...
          {(statusFilter || periodFilter) && (
            <button
              ...
              className="px-4 py-3 bg-white/[0.04] hover:bg-white/[0.08] border border-white/[0.06] text-white rounded-xl transition-all text-sm"
            >
              Шүүлтүүр цэвэрлэх
            </button>
          )}
          ...
          <table className="w-full min-w-[900px]">
            ...
            <tbody>
              {filtered.map((sub) => {
                ...
                return (
                  <tr key={sub.id} className="border-b border-white/[0.04] hover:bg-white/[0.04] transition-all">
