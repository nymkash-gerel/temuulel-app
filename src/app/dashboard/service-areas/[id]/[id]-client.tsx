...
export default function ServiceAreaDetailClient({ storeId }: { storeId: string }) {
          ...
          <div className="flex flex-wrap gap-2">
            {area.zip_codes && area.zip_codes.length > 0 ? (
              area.zip_codes.map((zip, idx) => (
                <span
                  key={idx}
                  className="px-3 py-1 bg-white/[0.04] border border-white/[0.06] rounded-full text-sm text-white font-mono"
                >
                  {zip}
                </span>
              ))
