...
export default function RepairOrderDetailClient({ storeId }: { storeId: string }) {
  ...
  const load = useCallback(async () => {
    ...
    if (!storeId) { router.push('/login'); return }
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex items-center gap-3">
          ...
          {!isEditing ? (
            <button
              onClick={startEdit}
              className="px-4 py-2 text-sm bg-white/[0.04] hover:bg-white/[0.08] border border-white/[0.06] text-white rounded-xl transition-all"
            >
              Засах
            </button>
          ) : (
              ...
              <div>
                ...
                {isEditing ? (
                  <textarea
                    ...
                    rows={3}
                    className="w-full px-3 py-2 bg-white/[0.04] border border-white/[0.06] rounded-lg text-white text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                  />
                ) : (
            ...
            {parts.length > 0 ? (
              <div className="overflow-x-auto">
                <table className="w-full min-w-[700px]">
                  ...
                  <tbody>
                    {parts.map((part) => (
                      <tr key={part.id} className="border-b border-white/[0.04]">
