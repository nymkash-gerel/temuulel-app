...
export default function UnitDetailClient({ storeId }: { storeId: string }) {
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          <button onClick={() => router.push('/dashboard/units')} className="p-2 text-slate-400 hover:text-white hover:bg-white/[0.08] rounded-lg transition-all">&larr; Буцах</button>
          <div>
          ...
          <div className="space-y-4 max-w-lg">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input value={formUnitNumber} onChange={e => setFormUnitNumber(e.target.value)} className="w-full px-4 py-3 bg-white/[0.04] border border-white/[0.06] rounded-xl text-white focus:outline-none focus:border-blue-500/30 focus:ring-2 focus:ring-blue-500/20" />
              </div>
              <div>
                <label className="block text-sm text-slate-400 mb-1">Төрөл</label>
                <select value={formUnitType} onChange={e => setFormUnitType(e.target.value)} className="w-full px-4 py-3 bg-white/[0.04] border border-white/[0.06] rounded-xl text-white focus:outline-none focus:border-blue-500/30 focus:ring-2 focus:ring-blue-500/20">
            ...
            <div className="grid grid-cols-3 gap-4">
              <div>
                ...
                <input type="number" value={formFloor} onChange={e => setFormFloor(e.target.value)} className="w-full px-4 py-3 bg-white/[0.04] border border-white/[0.06] rounded-xl text-white focus:outline-none focus:border-blue-500/30 focus:ring-2 focus:ring-blue-500/20" />
              </div>
              <div>
                <label className="block text-sm text-slate-400 mb-1">Хүний тоо</label>
                <input type="number" value={formMaxOccupancy} onChange={e => setFormMaxOccupancy(e.target.value)} className="w-full px-4 py-3 bg-white/[0.04] border border-white/[0.06] rounded-xl text-white focus:outline-none focus:border-blue-500/30 focus:ring-2 focus:ring-blue-500/20" />
              </div>
              <div>
                <label className="block text-sm text-slate-400 mb-1">Үнэ (₮)</label>
                <input type="number" value={formBaseRate} onChange={e => setFormBaseRate(e.target.value)} className="w-full px-4 py-3 bg-white/[0.04] border border-white/[0.06] rounded-xl text-white focus:outline-none focus:border-blue-500/30 focus:ring-2 focus:ring-blue-500/20" />
              </div>
            </div>
            <div>
              <label className="block text-sm text-slate-400 mb-1">Тохижилт (таслалаар)</label>
              <input value={formAmenities} onChange={e => setFormAmenities(e.target.value)} className="w-full px-4 py-3 bg-white/[0.04] border border-white/[0.06] rounded-xl text-white focus:outline-none focus:border-blue-500/30 focus:ring-2 focus:ring-blue-500/20" />
            </div>
            <div>
