...
export default function PackageDetailClient({ storeId }: { storeId: string }) {
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          <button onClick={() => router.push('/dashboard/packages')} className="p-2 text-slate-400 hover:text-white hover:bg-white/[0.08] rounded-lg transition-all">&larr; Буцах</button>
          <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Main info */}
        <div className="lg:col-span-2 bg-white/[0.04] rounded-xl p-6 border border-white/[0.06]">
          {editing ? (
            <div className="space-y-4 max-w-lg">
              ...
              <div>
                ...
                <textarea value={formDescription} onChange={e => setFormDescription(e.target.value)} rows={3} className="w-full px-4 py-3 bg-white/[0.04] border border-white/[0.06] rounded-xl text-white focus:outline-none focus:border-blue-500/30 focus:ring-2 focus:ring-blue-500/20 resize-none" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm text-slate-400 mb-1">Үнэ (₮) *</label>
                  <input type="number" value={formPrice} onChange={e => setFormPrice(e.target.value)} className="w-full px-4 py-3 bg-white/[0.04] border border-white/[0.06] rounded-xl text-white focus:outline-none focus:border-blue-500/30 focus:ring-2 focus:ring-blue-500/20" />
                </div>
                <div>
                  <label className="block text-sm text-slate-400 mb-1">Анхны үнэ (₮)</label>
                  <input type="number" value={formOrigPrice} onChange={e => setFormOrigPrice(e.target.value)} className="w-full px-4 py-3 bg-white/[0.04] border border-white/[0.06] rounded-xl text-white focus:outline-none focus:border-blue-500/30 focus:ring-2 focus:ring-blue-500/20" />
                </div>
              </div>
              <div>
                <label className="block text-sm text-slate-400 mb-1">Хүчинтэй хугацаа (хоног)</label>
                <input type="number" value={formValidDays} onChange={e => setFormValidDays(e.target.value)} className="w-full px-4 py-3 bg-white/[0.04] border border-white/[0.06] rounded-xl text-white focus:outline-none focus:border-blue-500/30 focus:ring-2 focus:ring-blue-500/20" />
              </div>
          ...
          {pkg.package_services && pkg.package_services.length > 0 ? (
            <div className="space-y-3">
              {pkg.package_services.map(ps => (
                <div key={ps.id} className="flex items-center justify-between p-3 bg-white/[0.04] rounded-lg">
