...
export default function NewPackageClient({ storeId }: { storeId: string }) {
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-8">
        <Link href="/dashboard/packages" className="p-2 text-slate-400 hover:text-white hover:bg-white/[0.08] rounded-lg">
              ...
              {packageServices.length === 0 ? (
                ...
                <div className="space-y-3">
                  {packageServices.map((ps, idx) => (
                    <div key={ps.id} className="flex items-center gap-3 p-3 bg-white/[0.04] rounded-xl">
                      ...
                      <select value={ps.service_id} onChange={(e) => updateService(ps.id, 'service_id', e.target.value)}
                        className="flex-1 px-3 py-2 bg-white/[0.04] border border-white/[0.06] rounded-lg text-white text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                      ...
                      <input type="number" value={ps.quantity} onChange={(e) => updateService(ps.id, 'quantity', parseInt(e.target.value) || 1)}
                        className="w-20 px-3 py-2 bg-white/[0.04] border border-white/[0.06] rounded-lg text-white text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
