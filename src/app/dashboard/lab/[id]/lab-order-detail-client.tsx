...
interface Props {
  initialLabOrder: LabOrderDetail
  initialResults: LabResult[]
  labOrderId: string
}

export default function LabOrderDetailClient({ initialLabOrder, initialResults, labOrderId }: Props) {
  const [labOrder, setLabOrder] = useState<LabOrderDetail>(initialLabOrder)
  ...
  const load = useCallback(async () => {
    const res = await fetch(`/api/lab-orders/${labOrderId}`)
    if (res.ok) {
      const data = await res.json()
      setLabOrder(data)
    }

    const resResults = await fetch(`/api/lab-results?order_id=${labOrderId}`)
    if (resResults.ok) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <StatusActions
          ...
          apiPath={`/api/lab-orders/${labOrderId}`}
          onSuccess={load}
        />
        {!isEditing ? (
          <button
