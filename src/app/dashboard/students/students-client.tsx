...
export default function StudentsClient({ storeId }: { storeId: string }) {
  ...
  if (loading) {
    return (
      <div>
        ...
        {/* Search skeleton */}
        <div className="mb-6">
          <div className="h-12 w-full max-w-sm bg-white/[0.04] rounded-xl animate-pulse" />
        </div>

        {/* Table skeleton */}
        <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl overflow-hidden">
          {/* Header row */}
          <div className="border-b border-white/[0.06] px-6 py-4 flex gap-6">
          ...
          {Array.from({ length: 8 }).map((_, rowIdx) => (
            <div
              key={rowIdx}
              className="border-b border-white/[0.04] px-6 py-4 flex gap-6"
            >
              {Array.from({ length: 6 }).map((_, colIdx) => (
                <div
                  key={colIdx}
                  className="h-4 flex-1 bg-white/[0.04] rounded animate-pulse"
      ...
      <div className="mb-6">
        <div className="relative max-w-sm">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-3 bg-white/[0.04] border border-white/[0.06] rounded-xl text-white text-sm placeholder-slate-400 focus:outline-none focus:border-blue-500/30 focus:ring-2 focus:ring-blue-500/20 transition-all"
          />
          {search && (
            <button
              onClick={() => setSearch('')}
            ...
            <div className="flex items-center justify-between mt-6">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  disabled={page === 0}
                  className="px-4 py-2 text-sm bg-white/[0.04] hover:bg-white/[0.08] border border-white/[0.06] text-white rounded-lg transition-all disabled:opacity-40 disabled:cursor-not-allowed"
                >
                  Өмнөх
                </button>
